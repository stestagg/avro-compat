  ...
  def collect(self):
    ...
    for case in self.suite:
      if isinstance(case, unittest.TestCase):
        collected.append(LoadTestsCase.from_parent(self, name=case.id(), item=case))
      elif isinstance(case, unittest.TestSuite):
        collected.append(
            LoadTestsSuiteCollector.from_parent(self, name='suite_child_of_mine', suite=case))
    return collected
...
def pytest_pycollect_makeitem(collector, name, obj):
  if name == 'load_tests' and isinstance(obj, types.FunctionType):
    ...
    try:
      ...
      return LoadTestsSuiteCollector.from_parent(collector, name=name, suite=suite)