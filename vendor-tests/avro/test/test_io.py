...
import avro.timezones  # switch-compat
        ...
        (
            {
                "type": "record",
                "name": "record",
                "fields": [{"name": "value", "type": "int"}, {"name": "next", "type": ["null", "record"]}],
            },
        ...
        (
            {
                "type": "record",
                "name": "ns.long",
                "fields": [{"name": "value", "type": "int"}, {"name": "next", "type": ["null", "ns.long"]}],
            },
            {"value": 0, "next": {"value": 1, "next": None}},
        ),
...
def write_datum(
    datum: object, writers_schema: avro.schema.Schema
) -> Tuple[io.BytesIO, avro.io.BinaryEncoder, avro.io.DatumWriter]:
    writer = io.BytesIO()
...
def read_datum(
    buffer: io.BytesIO, writers_schema: avro.schema.Schema, readers_schema: Optional[avro.schema.Schema] = None
) -> object:
...
class DefaultValueTestCase(unittest.TestCase):
    def __init__(
        self, field_type: Collection[str], default: Union[Dict[str, int], List[int], None, float, str]
    ) -> None:
        """Ignore the normal signature for unittest.TestCase because we are generating
        many test cases from this one class. This is safe as long as the autoloader
        ignores this class. The autoloader will ignore this class as long as it has
    ...
    def check_default_value(self) -> None:
        ...
        with warnings.catch_warnings(record=True) as actual_warnings:
            ...
            datum_read = (
                {"H": cast(bytes, datum_read_["H"]).decode()} if isinstance(datum_read_["H"], bytes) else datum_read_
            )
...
class TestMisc(unittest.TestCase):
    ...
    def test_type_exception_long(self) -> None:
        writers_schema = avro.schema.parse(
            json.dumps({"type": "record", "name": "Test", "fields": [{"name": "foo", "type": "long"}]})
        )
...
def load_tests(loader: unittest.TestLoader, default_tests: None, pattern: None) -> unittest.TestSuite:
    ...
    suite.addTests(
        ...
        for write_type, read_type in itertools.combinations(("int", "long", "float", "double"), 2)
    )
